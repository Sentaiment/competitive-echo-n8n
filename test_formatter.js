...
// Simulate the n8n input structure
const testData = JSON.parse(fs.readFileSync('test_scenario_data.json', 'utf8'));
...
// Set up global $input like n8n would
global.$input = mockInput;

// Execute the Prompt 32 Formatter logic
...
for (const scenario of scenarios) {
  ...
  try {
    ...
    console.log("Extracted competitors count:", scenarioData.competitors_ranked?.length || 0);
...
console.log("First scenario competitors:", processedScenarios[0]?.competitors_ranked?.length || 0);
console.log("Competitors:", processedScenarios[0]?.competitors_ranked?.map(c => c.company) || []);
